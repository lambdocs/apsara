...
(defn uncache-it! [node]
  "Remove node from cache"
  (reset! node-lookup (dissoc @node-lookup (picljid node))))

(defn annotate-node [node, parent-id]
...
(defn- append-child [parent node]
  "Append node as the right most child of parent."
  (conj parent node))

(defn- replace-child [parent index node]
...
(defn- splice-in [parent index node]
...
(defn- add-child [parent node right-sibling]
...
(defn- bubble-up [parent node right-sibling]
...
(defn insert-char-at! [id c offset]
...
(defn split-subtree-at [id offset]
...
(defn transplant! [nlist new-parent-id]
...
(defn enclose! [id enclosing-node]
